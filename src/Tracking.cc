...
Tracking::Tracking(System* pSys, ORBVocabulary* pVoc, FrameDrawer* pFrameDrawer, MapDrawer* pMapDrawer, Map* pMap,
        ...
        mState(NO_IMAGES_YET), mpORBVocabulary(pVoc), mpKeyFrameDB(pKFDB),
...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat& im, const double& timestamp)
{
    ...
    if(mState == NOT_INITIALIZED || mState == NO_IMAGES_YET)
        ...
        mCurrentFrame = Frame(mImGray, timestamp, mpORBextractor, mpORBVocabulary, mK, mDistCoef);
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    mpMapDrawer->SetCurrentCameraPose(pKFcur->GetPose(), pKFcur->mTimeStamp);
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    bool bLocalMappingIdle = mpLocalMapper->isAcceptKeyFrames();
...
void Tracking::UpdateLocalMap()
{
    ...
    mpMap->SetLocalMapPoints(mvpLocalMapPoints); // 添加的局部地图点为更新前的局部地图点
            ...
            if(!Tcw.empty())
            {
                ...
                for(int io = 0; io < mCurrentFrame.N; io++)
                {
                    if(mCurrentFrame.mvbOutlier[io])
                    {
                        ...
                        mCurrentFrame.mvbOutlier[io] = false;
                    }
                }
...
void Tracking::Reset()
{
    ...
    mpLocalMapper->RequestReset(); // 阻塞等待
    cout << " done" << endl;
