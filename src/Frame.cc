...
Frame::Frame(const Frame& frame)
        :mpORBvocabulary(frame.mpORBvocabulary), mpORBextractor(frame.mpORBextractor),
         ...
         mDescriptors(frame.mDescriptors.clone()), mvpMapPoints(frame.mvpMapPoints),
...
Frame::Frame(const cv::Mat& imGray, const double& timeStamp, ORBextractor* extractor, ORBVocabulary* voc, cv::Mat& K, cv::Mat& distCoef)
    :mpORBvocabulary(voc), mpORBextractor(extractor), mTimeStamp(timeStamp), mK(K.clone()), mDistCoef(distCoef.clone())
{
    // Frame ID
    mnId = nNextId++;
...
bool Frame::IsInImage(const float& x, const float& y)
{
