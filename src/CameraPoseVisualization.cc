...
void Eigen2Point(const Vector3d& v, geometry_msgs::Point& p) {
    p.x = v.x();
    p.y = v.y();
    p.z = v.z();
}
...
CameraPoseVisualization::CameraPoseVisualization(float r, float g, float b, float a)
    : m_marker_ns("CameraPoseVisualization"), m_scale(0.5), m_line_width(0.02) {
...
void CameraPoseVisualization::add_edge(const Vector3d& p0, const Vector3d& p1, float line_width){
...
void CameraPoseVisualization::add_loopedge(const Vector3d& p0, const Vector3d& p1){
    visualization_msgs::Marker marker;
...
void CameraPoseVisualization::add_pose(const Vector3d& p, const Quaterniond& q) {
    visualization_msgs::Marker marker;
